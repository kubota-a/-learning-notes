...
# 「(身長m)^2」の計算の仕方がよくわからなかったのでAIに聞いた　→　「身長（メートル単位） × 身長（メートル単位）」

# 設計メモ
# 変数weight_kgに58を格納、変数height_mに1.7を格納
# 変数bmiに、計算式「weight_kg　÷　height_mの二乗」を格納
# printでbmiを出力

weight_kg = 58
height_m = 170 / 100    # 直に「height_m = 1.7」と書いてしまった。
...
# input()で入力された文字列としての数字をfloat型にキャストすべきところを、int型にキャストしてしまってValueエラーが出た
